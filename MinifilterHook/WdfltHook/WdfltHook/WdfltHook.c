            ...
            if (InstanceList)
            {
                ...
                if (NT_SUCCESS(status))
                {
                    ...
                    for (ULONG i = 0; i < NumberOfInstancesReturned; i++)
                    {
                        ...
                        if (!NT_SUCCESS(status))
                        {
                            DbgPrint("[WdFilter_Hook] Failed to read instance memory!\n", i + 1);
                            ExFreePoolWithTag(CurrentInstanceVA, DRIVER_TAG);
                            break;
                        }
                        else
                        {
...

{
    ...
    FltUnregisterFilter( gFilterHandle );
    DbgPrint("[WdFilter_Hook] Driver unloaded !\n");
    return STATUS_SUCCESS;
}
